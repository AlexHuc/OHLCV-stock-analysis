#!/bin/bash
# Deployment script for OHLCV Stock Analysis Service

echo "🚀 Deploying OHLCV Stock Analysis Service"
...
kubectl wait --for=condition=ready pod -l app=ohlcv-prediction -n ohlcv-prediction --timeout=60s

# Show status
echo ""
echo "📊 Status:"
kubectl get pods -n ohlcv-prediction
kubectl get service -n ohlcv-prediction
...
kubectl port-forward service/ohlcv-prediction-service 9696:80 -n ohlcv-prediction &
PORT_FORWARD_PID=$!
...
echo "   (Select 'ohlcv-prediction' namespace)"
echo ""
